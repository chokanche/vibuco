...
The current product is not operationally trustworthy. The production `/cards` route returned a 502 during this audit. The repository has no automated tests, CI, lockfile, analytics, error reporting, or infrastructure definition. Protected content is read by the browser through Cognito identity credentials, a DynamoDB table scan, and S3 object downloads. The card route ships 927 kB of first-load JavaScript, while the global CSS bundle is 293 kB because Tailwind purging is not configured.
